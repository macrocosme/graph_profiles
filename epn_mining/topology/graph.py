...
class Graph:
    ...
    def get_n_per_level(self):
        self.n_per_level = np.histogram(self.farness[mst.centrality.argmax()],
                                        bins=np.unique(self.farness[mst.centrality.argmax()]))
        return self.n_per_level

    def compute_closeness_centrality(self):
        edges = self.get_mst_edges_dict()
        N = len(edges.keys())
        k = N-1
        self.farness = np.zeros([N, N])
        ...
        self.centrality = k/self.farness.sum(axis=1)
        return self.centrality
    ...
    def elongation(self):
        ...
        # l = self.length()
        # l = self.l
        l = len(self.longest_path)
        w = self.half_width()

        # print  (l, w, self.V)

        return l/(2*w), l/(2*w*self.V)

    def length(self):
        if self.farness is None:
            _ = self.compute_closeness_centrality()

        return self.farness.mean(axis=1)[self.find_least_connected_vertex()]

    def half_width(self, start_node=0):
    ...
    def get_nodes_degrees(self, return_type='dataframe'):
        # Types: 'dataframe', otherwise returns a dict
